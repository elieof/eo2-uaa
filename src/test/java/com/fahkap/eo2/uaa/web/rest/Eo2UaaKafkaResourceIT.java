...
import com.fahkap.eo2.uaa.config.KafkaProperties;
...
class Eo2UaaKafkaResourceIT {
    ...
    void setup() {
        ...
        Eo2UaaKafkaResource kafkaResource = new Eo2UaaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eo-2-uaa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/eo-2-uaa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
